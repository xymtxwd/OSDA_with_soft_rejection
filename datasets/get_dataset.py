...
def relabel_dataset_(train_dataset, test_dataset, task):
    image_path = []
    image_label = []
    if task == 's2m':
        ...
        for i in range(len(train_dataset.train_data)):
            if int(train_dataset.train_labels[i]) >= 5:
                train_dataset.train_labels[i] = 5
                #image_label.append(5)
...
def relabel_dataset(train_dataset, test_dataset, task):
    ...
    if task == 's2m':
        for i in range(len(train_dataset.data)):
            if int(train_dataset.labels[i]) < 4:
                image_path.append(train_dataset.data[i])
                image_label.append(train_dataset.labels[i])
            elif int(train_dataset.labels[i]) < 7:
                image_path.append(train_dataset.data[i])
    ...
    if task == 'u2m':
        for i in range(len(train_dataset.train_data)):
            if int(train_dataset.train_labels[i]) < 4:
                image_path.append(train_dataset.train_data[i])
                image_label.append(train_dataset.train_labels[i])
            elif int(train_dataset.train_labels[i]) < 7:
                image_path.append(train_dataset.train_data[i])
                image_label.append(np.int32(4))
        train_dataset.train_data = image_path
        train_dataset.train_labels = image_label
    if task == 'm2u':
        for i in range(len(train_dataset.train_data)):
    ...
    if task == 'm2u':
        for i in range(len(test_dataset.train_data)):
            if int(test_dataset.train_labels[i]) < 4:
                ...
                test_image_path.append(test_dataset.train_data[i])
                test_image_label.append(np.int32(4))
