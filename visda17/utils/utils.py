...
def get_model(net, num_class=13, unit_size=100):
    if net == 'alex':
        model_g = AlexBase()
        model_c1 = Classifier(num_classes=num_class)
        model_c2 = Classifier(num_classes=num_class)
    elif net == 'vgg':
        model_g = VGGBase()
        model_c1 = Classifier(num_classes=num_class)
        model_c2 = Classifier(num_classes=num_class)
        ...
        model_c1 = ResClassifier(num_classes=num_class, unit_size=unit_size)
        model_c2 = ResClassifier(num_classes=num_class, unit_size=unit_size)
